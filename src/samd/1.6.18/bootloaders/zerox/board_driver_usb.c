...
void USB_Init(void)
{
  ...
  GCLK->CLKCTRL.reg = GCLK_CLKCTRL_ID( 6 ) | // Generic Clock Multiplexer 6
              GCLK_CLKCTRL_GEN_GCLK1 | // Generic Clock Generator 0 is source
              GCLK_CLKCTRL_CLKEN ;
...
uint32_t USB_Read_EP(Usb *pUsb, char *pData, uint32_t length, uint32_t ep)
{
  uint32_t packetSize = 0;
  if (!read_job)
  {
    /* Set the buffer address for ep data */
    usb_endpoint_table[ep].DeviceDescBank[0].ADDR.reg = (uint32_t)&udd_ep_out_cache_buffer[ep-1];
    /* Set the byte count as zero */
  ...
  if ( pUsb->DEVICE.DeviceEndpoint[ep].EPINTFLAG.bit.TRCPT0 )
  {
    /* Set packet size */
    packetSize = SAM_BA_MIN(usb_endpoint_table[ep].DeviceDescBank[0].PCKSIZE.bit.BYTE_COUNT, length);
    /* Copy read data to user buffer */
    memcpy(pData, udd_ep_in_cache_buffer[ep-1], packetSize);
...
uint32_t USB_Read_blocking_EP(Usb *pUsb, char *pData, uint32_t length, uint32_t ep)
{
...
/*    
	...
	return read;
  */
  //pUsb->DEVICE.DeviceEndpoint[ep].EPSTATUSSET.bit.BK0RDY = true;
  uint32_t packetSize = 0;

  /* Set the buffer address for ep data */
  usb_endpoint_table[ep].DeviceDescBank[0].ADDR.reg = (uint32_t)pData;
  /* Set the byte count as zero */
  /* Set the multi packet size as zero for multi-packet transfers where length > ep size */
  usb_endpoint_table[ep].DeviceDescBank[0].PCKSIZE.bit.MULTI_PACKET_SIZE = 8;//length;
  usb_endpoint_table[ep].DeviceDescBank[0].PCKSIZE.bit.BYTE_COUNT = 0;
  /* Clear the bank 0 ready flag */
  pUsb->DEVICE.DeviceEndpoint[ep].EPSTATUSCLR.bit.BK0RDY = true;
  /* Wait for transfer to complete */
  while (!( pUsb->DEVICE.DeviceEndpoint[ep].EPINTFLAG.bit.TRCPT0 ));
  /* Clear Transfer complete 0 flag */
    /* Copy read data to user buffer */
  //memcpy(pData, udd_ep_out_cache_buffer[USB_EP_OUT-1], length);
...
uint8_t USB_IsConfigured(P_USB_CDC pCdc)
{
  ...
  else
  {
    ...
    }else if(pUsb->DEVICE.DeviceEndpoint[0].EPINTFLAG.bit.TRCPT0){
       /* memcpy(&line_coding_in,udd_ep_out_cache_buffer[0],7);
        GPIO_ON(19);
       
       if(line_coding_in.dwDTERate==115200){
           GPIO_ON(12);
           GPIO_ON(16);
           GPIO_ON(17);
           //GPIO_OFF(16);
        }
        if(line_coding_in.dwDTERate==1200){
           GPIO_ON(12);
           GPIO_ON(16);
           GPIO_ON(17);
           GPIO_OFF(12);
        ...
        if(line_coding_in.dwDTERate==57600){
           GPIO_ON(12);
           GPIO_ON(16);
           GPIO_ON(17);
           GPIO_OFF(17);
